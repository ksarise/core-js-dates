...
function getNextFriday(date) {
  ...
  const nextFriday = new Date(date.setDate(date.getDate() + nextDays));
  return nextFriday;
}
...
function formatDate(date) {
  ...
  const dayTime = unFormatDate.toLocaleTimeString('en-US', { timeZone: 'UTC' });
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= days; i += 1) {
    const currDay = new Date(year, month - 1, i);
...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil(
    ((date - yearStart) / (1000 * 60 * 60 * 24) + 1 + firstDay) / 7
  );
}
